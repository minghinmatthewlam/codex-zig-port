...
- CI: GitHub Actions runs formatting, Python smoke-script compilation, unit
  ...
  `77e255322060b7b9f6f0b08693ca0ddcf13ab8fa`; branch pushes do not start CI
...
- Fresh public clone proof: a clean HTTPS clone from
  ...
  `77e255322060b7b9f6f0b08693ca0ddcf13ab8fa` passed `python3 -m py_compile`
  ...
  `zig build test --summary all` with `658/658` tests passed,
  `zig build e2e --summary all`, and
  `python3 scripts/real_computer_use_mcp_smoke.py zig-out/bin/codex-zig`
  with the bundled `computer-use` plugin calling `get_app_state`.
