...
- Security settings: GitHub API checks through 2026-05-17 show secret scanning,
  ...
  CodeQL run `25989225768` passed on pushed head
  `9be4ebc69e20af2ddc21bde4cd2d7f50b31ef613`, and a follow-up
  code-scanning alerts API query reported no open alerts. Dependabot and
  secret-scanning alert API queries also returned no open alerts.
...
- CI: GitHub Actions runs formatting, Python smoke-script compilation, unit
  ...
  Checked push run `25989225749` passed on pushed head
  `9be4ebc69e20af2ddc21bde4cd2d7f50b31ef613`. Local pre-push verification for
  that slice included `python3 -m py_compile scripts/tui_e2e.py`, `zig fmt
  --check src/git_apply.zig src/cloud_cmd.zig src/apply_command.zig`, `git diff
  --check`, `zig build test --summary all`, `zig build e2e --summary all`, and
...
- Source hygiene: current tracked-file and hidden working-tree scans found no
  ...
  dummy credentials in smoke coverage. `gitleaks` and the local CodeQL CLI were
  not installed on the local machine during the latest check, so the local scan
  used repository `rg` patterns plus GitHub's enabled secret scanning and CodeQL
  runs.
