...
`processId`, active `command/exec/write` stdin streaming, stdio,
...
roots. Fully deferred non-stream command responses and full Rust async command
session parity remain planned. Inactive follow-up calls still return
Rust-shaped inactive-process errors. Empty
