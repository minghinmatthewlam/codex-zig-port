...
| `apply-patch` | partial | Exposes an `apply_patch` function tool with approval and a Zig-native parser for add, update, delete, move-to, multiple hunks, EOF markers, padded markers, no-newline replacements, and Rust-compatible `Add File` overwrite behavior for existing regular files. The PTY TUI E2E verifies an approved model-requested add-file patch creates the expected file on disk. Full grammar and fixture parity are still planned. |
