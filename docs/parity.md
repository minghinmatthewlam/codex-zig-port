...
`thread/backgroundTerminals/clean` now returns the Rust-shaped empty response
for already-loaded threads in the current no-background-terminal runtime while
preserving invalid and missing-thread errors. Actual background terminal
tracking and termination remain planned.
