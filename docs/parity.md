...
`archive`, `delete`, `unarchive`, `responses-api-proxy`, and `stdio-to-uds`,
and root `help stdio-to-uds` resolves to the hidden command's local help.
...
Current local TUI `/experimental` coverage: `/experimental` lists the Rust
menu-visible experimental features with effective enabled state, Rust menu
label, description, and config key. `/experimental enable FEATURE` and
`/experimental disable FEATURE` persist the active profile's feature config and
refresh the displayed list. Rust's richer popup keyboard navigation and live
feature-application semantics remain planned TUI depth.

Current local TUI `/apps` coverage: `/apps` lists local plugin-provided apps
for the current working directory with installed/available counts, enabled
state, app id, description, install URL, and contributing plugin display names.
Rust's richer popup UI, remote app directory refresh, app auth/linking flows,
and `$` app insertion remain planned TUI/plugin depth.

Current local TUI `/plugins` coverage: `/plugins` lists local marketplace
plugins for the current working directory with marketplace display names,
paths, installed/available counts, plugin enabled/installed state, plugin id,
short or long description, category, capabilities, keywords, and load errors.
Rust's richer tabbed popup UI, search, install/uninstall, marketplace
add/remove/upgrade, remote catalogs, plugin details, app auth flow, and share
management remain planned TUI/plugin depth.

Current local TUI `/memories` coverage: `/memories` reports memory feature
enablement, current `use_memories` and `generate_memories` settings, persists
feature enable/disable and memory toggle changes to `config.toml`, and clears
local memory files through the same reset implementation used by
`debug clear-memories`. Rust's richer settings popup and reset confirmation UI
remain planned TUI depth.

Current local TUI `/ide` coverage: `/ide`, `/ide on`, `/ide off`, and
`/ide status` toggle or report IDE context. Enabling connects to the local
Codex IPC Unix socket, requests Rust-shaped `ide-context` for the current
workspace, renders active file, active selection, and open tabs into the shared
IDE prompt format, and prefixes subsequent local prompts before they are sent to
the model. The PTY smoke drives the rebuilt binary against a mock IDE IPC server
and verifies the mock Responses API receives the prefixed prompt. Rust's rich
status indicator, Windows pipe transport, and deeper transcript display/replay
trimming remain planned TUI depth.
...
custom-CA network env checks, passive app-server daemon state and
control-socket reachability, terminal streams, state paths, config-backed
...
trusted `SessionStart` and `UserPromptSubmit` command hooks, sends Rust-shaped
stdin payloads, emits `hook/started` / `hook/completed` lifecycle
...
for the next accepted turn, keeps untrusted or disabled hooks inert by default,
and honors `--dangerously-bypass-hook-trust` by running enabled untrusted hooks
for that invocation while preserving disabled state. Non-interactive
`exec`/`exec resume`/`exec review` also run trusted `SessionStart` and
`UserPromptSubmit` command hooks, pass accepted hook context as developer
messages, and honor the hook-trust bypass flag for that invocation.
...
app-server proxy, and hidden stdio-to-UDS relay smoke paths. Current
app-server line transports accept large JSON-RPC requests needed to reach
Rust's turn input validator; `turn/start` and `turn/steer` reject text input
over 1048576 characters with JSON-RPC `-32602` plus structured
`input_too_large` data instead of terminating the transport.
...
Runtime emission now covers trusted `SessionStart` and `UserPromptSubmit`
command hooks during `turn/start`; other hook events and full hook outcome
...
`request_permissions` permission request is answered, when a stdio MCP
elicitation request is answered during `mcpServer/tool/call` or `turn/start`,
and when an in-flight approval or turn-correlated elicitation is interrupted.
Runtime emission is also covered
for `mcpServer/startupStatus/updated` starting, ready, and failed notifications
while app-server `thread/start` and `thread/resume` validate required MCP
servers and report optional server startup in the background, and while
`turn/start` discovers configured MCP tools. Required-server `thread/start` and
`thread/resume` failure reporting is covered, as is
...
enum schema helpers. Runtime stdio MCP elicitation dispatch is covered through
direct `mcpServer/tool/call` and model-triggered `turn/start` MCP tool calls;
streamable HTTP tool-call clients now advertise elicitation capability and can
bridge `elicitation/create` requests from direct SSE responses through
app-server `mcpServer/elicitation/request`. Broader JSON Schema parity remains
planned.
...
states. Enabled local plugin-cache streamable HTTP servers are covered with
the same live tool, resource, resource-template, session, and bearer-token
inventory behavior. `config/mcpServer/reload` now validates request shape, reloads the
current config, and strictly rebuilds configured plus enabled plugin MCP server
inventory before returning an empty Rust-shaped response object. Thread-start
startup status notifications are covered separately; queued thread-owned MCP
manager refreshes after reload remain planned.
...
returns unavailable errors for disabled servers. Enabled local plugin-cache
stdio and streamable HTTP MCP resource reads are covered through the same
app-server RPC surface. MCP server JSON-RPC errors are forwarded with their
original code, message, and data payload. Streamable HTTP
...
forwarded with their original code, message, and data payload. Stdio MCP
`elicitation/create` requests are bridged to app-server
`mcpServer/elicitation/request`, resolved with the client response, and followed
by `serverRequest/resolved`. Streamable HTTP tool-call clients advertise
`2025-06-18` plus `elicitation` capability when an app-server elicitation bridge
...
comma-separated `--scopes` values, discovers OAuth metadata, dynamically
...
OAuth-discovery not-logged-in servers. Runtime streamable HTTP MCP clients use
...
`auto` or `keyring`. If a provider rejects metadata-discovered scopes,
`codex-zig mcp login` retries once without scopes. Cross-platform keyring
backends remain planned.

Additional model-facing MCP coverage: Responses turns now advertise configured
and enabled local plugin-cache stdio/streamable HTTP MCP tools plus the
`list_mcp_resources`, `list_mcp_resource_templates`, and `read_mcp_resource`
function tools. The runtime lists resources and templates from configured and
enabled local plugin-cache stdio/streamable HTTP MCP servers, supports
explicit-server pagination cursors, aggregates all enabled stdio and streamable
HTTP servers when no server is specified, reads config-backed and enabled local
plugin-cache stdio/streamable HTTP resources, discovers streamable HTTP
...
back through the shared session tool path. Stdio MCP `elicitation/create`
requests raised during model-triggered MCP tool calls are surfaced through
...
app-server deferred buffered responses, capture-time independent stdout/stderr
output-cap truncation, disabled output caps, timeout exit-code responses with
captured pre-timeout output, `streamStdoutStderr` output-delta notifications,
stdio, Unix-socket, and websocket app-server deferred command sessions with
client-supplied `processId`, active `command/exec/write` stdin streaming, stdio,
Unix-socket, and websocket macOS PTY-backed `tty` command sessions with output
deltas and active `command/exec/resize`, and active `command/exec/terminate`
handling,
...
project-roots workspace-write, `tmpdir` and `slash_tmp` writable roots,
project/current-working-directory writable subpaths, absolute writable-root,
non-root read-only allowlists for concrete readable roots, `:minimal` platform
defaults, parent-deny carveouts, concrete absolute or special-path read-deny
roots, and external profiles, with command-cwd sandbox rooting for project roots.
...
roots. Full Rust async command session parity remains planned. Inactive
follow-up calls still return Rust-shaped inactive-process errors. Empty
`processId` values on follow-up calls preserve Rust validation order:
payload/size errors win first, otherwise the inactive-command error includes
the empty id.
...
Unix-socket, and websocket app-server runtime coverage also supports active
async `process/spawn` sessions for `streamStdoutStderr`, `streamStdin`, and
macOS `tty`, active `process/writeStdin`, duplicate active-handle rejection,
active `process/kill`, active `process/resizePty`, streamed process output
deltas, and process-exit notifications. Broader Rust process lifecycle polish
remains planned. Empty
...
| --- | --- | --- |
| `cli` base interactive command | partial | First milestone launches `codex-zig` interactive loop, accepts an optional initial prompt, rejects unknown top-level flags, supports `--` before prompt text, exposes `help [COMMAND]`, Rust-shaped `help help`, nested root `help exec <subcommand>` for the implemented exec help topics, nested root `help debug <subcommand>` for the implemented debug help topics, nested root `help mcp <subcommand>` for the implemented MCP help topics, nested root `help plugin <subcommand>` for the implemented plugin and plugin marketplace help topics, and nested root `help cloud <subcommand>` for the implemented Cloud command family, includes top-level `apply` / `a`, includes local top-level `archive`, `delete`, and `unarchive` session-management commands for saved rollouts, includes `remote-fork CODE` for Rust-compatible local HTTP fork-claim imports, includes `update` help, Rust-compatible debug-build rejection, and release-build install-method detection for npm, bun, Homebrew, and standalone layouts, includes `app [PATH]` help plus macOS Codex Desktop open/install command parity, includes `exec-server --listen stdio` plus default/explicit `ws://IP:PORT` websocket transport with a Rust-shaped JSON-RPC `initialize` response, detached websocket `resumeSessionId` reconnects for process sessions, pipe-backed and macOS PTY-backed `tty` `process/start`, `process/read`, `process/write`, and `process/terminate` lifecycle, Rust-shaped `envPolicy` child process environment inheritance/filtering/set/include-only semantics with request `env` overlays, Unix `arg0` child process title override support, exec-server filesystem `fs/readFile`, `fs/writeFile`, `fs/createDirectory`, `fs/getMetadata`, `fs/readDirectory`, `fs/remove`, and `fs/copy` RPCs with supported managed filesystem sandbox contexts including glob-pattern deny entries, exec-server `http/request` RPCs for built-in HTTP methods with buffered and streamed timeout handling plus streamed `http/request/bodyDelta` response notifications over stdio and websocket transports, and exec-server remote registry registration through `--remote URL --executor-id ID [--name NAME]` with `CODEX_EXEC_SERVER_REMOTE_BEARER_TOKEN`, Rust-shaped registration payloads, and local `ws://` plus loopback self-signed or public-CA `wss://` rendezvous JSON-RPC serving, includes hidden `responses-api-proxy` loopback forwarding for `POST /v1/responses` with stdin bearer auth, server-info output, HTTP shutdown, streaming response forwarding, and JSON dump output with sensitive-header redaction, parses the Rust top-level `remote-control` command family with Rust-shaped option handling and the current state-db-unavailable headless runtime error, implements `cloud` / `cloud-tasks` command subcommands plus a line-oriented Cloud task picker, treats unknown prompt-like top-level names such as `marketplace` as initial prompts, preserves the common `NAME --help|--version` root global flag forms, consumes valid root interactive options after prompt-like names, rejects unknown tail flags and extra bare prompt arguments, lets help/version output take precedence over strict-config unknown overrides, dangerous-bypass approval conflicts, invalid pre-help `-C` directories, and root remote rejection for command help paths, and supports global `-m/--model`, `-i/--image`, `--enable`, `--disable`, `--oss`, `--local-provider`, `-p/--profile`, `-c/--config`, `--strict-config`, `-a/--ask-for-approval`, `-s/--sandbox`, `-C/--cd`, `--add-dir`, `--search`, `--remote`, `--remote-auth-token-env`, `--remote-control`, `--remote-control-bind`, `--no-alt-screen`, `--version`, `--yolo`, `--dangerously-bypass-approvals-and-sandbox`, and `--dangerously-bypass-hook-trust` overrides, including Rust-compatible rejection when dangerous bypass is combined with an explicit approval policy. `--profile` now uses the current Rust overlay semantics, while legacy profile selection stays available through `-c profile=NAME`; obsolete `--profile-v2` is rejected. Remote app-server flags and local remote-control flags are parsed and rejected for non-interactive subcommands; `--remote unix://PATH` plus loopback `--remote ws://HOST:PORT` and TLS `--remote wss://HOST:PORT` run a minimal remote TUI against an app-server for new text/local-image turns, remote resume/fork pickers, `resume --last`, explicit `resume TARGET`, explicit `fork ID|PATH`, `fork --last`, and websocket capability-token auth through `--remote-auth-token-env`; and `--remote-control` plus runtime `/remote-control [start|stop]` start and manage a minimal local browser controller with `/api/state`, live `/api/events` server-sent state snapshots, and controller-only `/api/message` prompt submission into the running TUI. The full Rust phone fork/share UI surface and headless remote-control transport remain planned. Full command behavior parity is planned. |
| `tui` terminal UI | partial | Current UI is a simple terminal surface with composer/transcript/tool status, alternate-screen enter/leave by default with `--no-alt-screen` inline mode, streamed assistant text deltas, `!COMMAND` local shell execution, and local `/help`, `/init`, `/compact`, `/status`, `/remote-control`, `/debug-config`, `/keymap`, `/plan`, `/agent`, `/subagents`, `/title`, `/statusline`, `/theme`, `/pets`, `/pet`, `/realtime`, `/settings`, `/personality`, `/experimental`, `/apps`, `/plugins`, `/memories`, `/ide`, `/approve`, `/feedback`, `/rename`, `/model`, `/fast`, `/permissions`, `/approval`, `/sandbox`, `/history`, `/mention`, `/side`, `/btw`, feature-gated `/goal`, `/rollout`, `/sessions`, `/diff`, `/copy`, `/raw`, `/vim`, `/mcp`, `/ps`, `/stop`, `/clean`, `/logout`, `/review`, `/clear`, `/new`, `/resume`, `/fork`, `/quit`, and `/exit` commands. `scripts/tui_e2e.py` drives the rebuilt binary in a pseudo-terminal against a mock Responses server for top-level `-i/--image` initial-prompt attachments, interactive help/status, effective config display with local config-source status, keymap listing/debug, plan-mode requests that omit tools and render `<proposed_plan>` blocks without leaking tags, `update_plan` checklist rendering with `task-progress` status/title items, token/context status-line and terminal-title item previews from response usage metadata, ordered terminal-title item rendering with OSC output/clearing and persistence to `[tui].terminal_title`, text-mode status-line item previews and persistence to `[tui].status_line`, syntax-theme listing/selection and persistence to `[tui].theme`, text-mode `/pets` built-in id listing and `[tui].pet` selection/disable persistence, feature-gated text-mode `/realtime` local active/inactive state plus `/status` reporting, feature-gated text-mode `/settings` realtime microphone/speaker selection and `[audio]` persistence, text-mode `/agent` and `/subagents` no-agent/disabled-state handling, personality listing/selection and persistence with request instructions, text-mode `/experimental` feature listing and config persistence, text-mode `/apps` local plugin app catalog listing, text-mode `/plugins` local marketplace plugin listing, text-mode `/memories` feature/settings persistence and memory reset, text-mode `/ide` IPC context toggling and prompt prefixing, text-mode `/approve` no-denials guidance, text-mode `/feedback` category/help/upload flow with local direct upload and remote app-server upload coverage, persisted thread renaming and session-list title display, basic persisted `/goal` set/view/edit-hint/clear/pause/resume handling when the `goals` feature is enabled, service-tier toggling, last-response copying, raw output toggling, Vim mode toggling, pending file mentions injected into API requests, Rust's 1 MiB text-character input-limit rejection for local normal prompts, initial prompts, local remote-control submissions, `/side` / `/btw`, and custom `/review`, prompted ephemeral `/side` and `/btw` turns that leave the main transcript unchanged, MCP status, explicit local shell commands, model and permissions updates, history rendering, runtime remote-control start/re-show/usage/stop handling, configured `review_model` for `/review` turns, approved `exec_command` and `apply_patch` calls, real file creation in a temporary workspace, background terminal cleanup through `/clean` plus silent `/quit`/EOF cleanup of local TUI-owned background terminals, fresh-launch preference loading, and API request coverage. Full Rust config layer stack rendering, configurable keymaps, rich alternate-screen layout, transcript overlay, slash popup, resize behavior, true syntax highlighting, rich plan implementation picker UI, rich status/title picker UI, rich goal editor/accounting UI, rich subagent picker/thread switching/lifecycle display, empty side-conversation UI, rich pet picker/preview/image rendering, rich realtime audio device picker, actual realtime microphone capture/speaker playback/WebSocket/WebRTC session runtime, and snapshots are planned. |
| `login` / local auth reuse | partial | Reads `$CODEX_HOME/auth.json` / `~/.codex/auth.json`, ChatGPT bearer token, ChatGPT account header, refreshes expired ChatGPT access-token JWTs or auth last refreshed more than 8 days ago with stored refresh tokens, supports Rust-compatible browser callback login, browser-login forced-workspace restrictions, browser-login best-effort token-exchange API key persistence, `agent_identity` access-token auth, API-key fallback, `CODEX_ACCESS_TOKEN`, `login status`, `login --with-api-key`, `login --with-access-token`, `login --device-auth`, and `logout` with best-effort managed ChatGPT OAuth revoke. The Rust `cli_auth_credentials_store` config is parsed, and file, macOS keychain, auto, and process-local ephemeral storage modes are wired through CLI login/status/logout plus config-aware auth consumers; app-server `chatgptAuthTokens` external auth stays process-local and takes precedence over persistent auth until logout clears both. Cross-platform keyring backends and full refresh error taxonomy are planned. |
| `core` Responses agent loop | partial | Sends Responses API requests, appends discovered `AGENTS.md` / `AGENTS.override.md` project docs to instructions, can expose the native `web_search` tool in cached/live mode, emits model-catalog-gated `text.verbosity` from configured or bundled default verbosity, advertises configured stdio and streamable HTTP MCP tools as function tools or, when `tool_search_always_defer_mcp_tools` is enabled, defers them behind a client-executed `tool_search` tool that returns namespace-shaped MCP tool specs and persists `tool_search_call` / `tool_search_output` history, exposes the stable v1 multi-agent discovery surface through the same capability-gated, client-executed `tool_search` path with the `multi_agent_v1` namespace and `spawn_agent`, `send_input`, `resume_agent`, `wait_agent`, and `close_agent` child specs when `[features].multi_agent` is enabled, runs a basic in-process v1 subagent runtime for stateful local TUI, app-server loaded-thread, and `mcp-server` `codex-reply` sessions where `spawn_agent` runs a child turn and returns a UUID v7-shaped agent id, exposes Rust-shaped inherited-model guidance plus bundled model/reasoning/service-tier override summaries in the `spawn_agent` tool description, applies service-tier child overrides plus non-forked model and reasoning-effort child overrides with bundled model-catalog defaulting/validation and unsupported inherited service-tier filtering, accepts `spawn_agent` and `send_input` structured `items` containing text plus URL-backed `image`, readable `local_image`, registered local `skill`, and structured `mention` content, converts unreadable local images into model-visible read-error placeholders, injects readable registered skill files as Rust-shaped `<skill>` user fragments while treating unregistered skill paths as model-visible read errors, preserves mention targets as model-visible markers, injects Rust-shaped developer guidance for app-server `plugin://...` mentions that resolve to enabled local plugins with visible skill, MCP, or app capabilities, rejects forked `agent_type`, model, and reasoning-effort overrides like Rust, `wait_agent` returns Rust-shaped final completed/not-found statuses, `send_input` continues an open child with a UUID v7-shaped submission id, `close_agent` marks it shutdown, and `resume_agent` reopens shutdown children for follow-up input, while non-stateful turns without a runtime do not advertise these subagent tools and child turns strip parent-scoped callbacks/features before running, rejecting unsupported role overrides, exposes model-facing stdio and streamable HTTP MCP resource tools (`list_mcp_resources`, `list_mcp_resource_templates`, and `read_mcp_resource`), exposes built-in `update_plan`, exposes app-server loaded-thread `get_goal`, `create_goal`, and `update_goal` tools when `[features].goals` is enabled with Zig transcript-backed persistence and local state-DB-backed goal snapshots for persistent loaded threads, parses text deltas, token usage, function calls, and client `tool_search_call` items, optionally streams text deltas to callers, loops tool output back, and supports manual TUI compaction into a continuation summary. Automatic compaction, remote compaction protocol parity, rollout persistence, full dynamic model catalog and cache/remote fallback for child overrides, automatic goal continuation, remote goal-store parity, background concurrent subagent execution, persisted subagent threads, lifecycle notifications/hooks, role config, app mention activation, multi-agent v2 tools, and advanced prompts are planned. |
...
| `config` | partial | Resolves Codex home, installation id, model/base URL, top-level `review_model` for review turns, top-level `model_context_window`, top-level `model_auto_compact_token_limit`, top-level/profile-scoped `model_verbosity`, `model_provider` with `[model_providers.<name>].base_url`, provider `wire_api`, provider `env_key`, provider `experimental_bearer_token`, provider `query_params`, provider `http_headers` / `env_http_headers`, and section-form or inline-table command auth including `refresh_interval_ms`, `approval_policy`, `sandbox_mode`, `web_search`, `model_reasoning_effort`, `service_tier`, Rust-compatible top-level `instructions`, `developer_instructions`, `compact_prompt`, `[tui].theme`, `[tui].status_line`, `[tui].terminal_title`, `[tui].alternate_screen`, `[audio].microphone`, `[audio].speaker`, legacy `syntax_theme`, `personality`, `oss_provider`, `background_terminal_max_timeout`, supported custom `[permissions]` profiles, and active profile selection from env/config. Model-facing CLI paths use configured provider env-key, bearer-token, or command-backed auth before stored Codex auth; the CLI smoke verifies `exec` sends the provider env-key token to the configured provider base URL, routes `wire_api = "responses"` to `/responses`, appends configured provider query params to the request URL, attaches configured static and env-backed provider headers, rejects removed `wire_api = "chat"` before issuing a request, uses section-form plus inline command-auth stdout as a bearer token, caches command-auth tokens while fresh, proactively reruns command auth after `refresh_interval_ms`, reruns command auth after a 401 and retries with the refreshed token, and rejects Rust-invalid command-auth conflicts. Supports top-level `profile`, `[profiles.<name>]`, legacy profile selection through `-c profile=NAME`, current Rust `--profile NAME` user-config overlays on root runtime commands plus local `exec --profile`, including Rust-style plain-name validation and legacy-profile conflict rejection, and Rust-compatible `-c/--config key=value` for supported scalar fields the Zig port currently understands. Root interactive/resume/fork, exec, and review `--strict-config` now reject unknown user `config.toml` fields and unknown `-c/--config` override fields, including unknown feature keys and nested MCP/provider leaf fields while accepting Rust-known opaque desktop settings. App-server config writes accept optional `filePath` only when it resolves to the user `config.toml`, reject non-user absolute paths, and return Rust-shaped `okOverridden` metadata when supported user scalar writes are masked by legacy managed config. Full TOML config stack, full strict-config/profile-overlay command-surface/layer edge cases, full custom permissions-profile enforcement, full project config stack, full override metadata, and full managed/cloud requirements enforcement are planned. |
| `features` | partial | Supports `codex-zig features list` with the current Rust feature keys, stages, default states, simple `[features]` boolean overrides from `config.toml`, profile-scoped `[profiles.NAME.features]` overrides, Rust legacy aliases such as `collab` / `memory_tool` mapped onto canonical effective feature state, root/list-local runtime `--enable/--disable FEATURE` overrides, Rust-compatible `-c/--config` feature overrides for flat `features.<key>=...`, nested `features.<key>.enabled=...` leaves, `features.apps_mcp_path_override.path` payloads from config and runtime `-c` overrides that drive the host-owned `codex_apps` MCP URL, and inline `features={...}` TOML tables on the user-visible command surfaces that accept feature controls, root app-server feature overrides through `experimentalFeature/list` and `config/read`, and `features enable|disable FEATURE` writes to either the top-level `[features]` table or profile-scoped feature table when root `-c profile=NAME` is set. Unsupported nested runtime payload leaves such as `features.network_proxy.*` settings and non-toggle `multi_agent_v2` settings are rejected until their runtime consumers are ported instead of being accepted and dropped. The local feature catalog is aligned with the installed Rust `codex-cli 0.145.0` default `features list` output, including `mentions_v2`, `plugin_sharing`, removed/stable stage changes, and experimental menu metadata for TUI display. App-server `experimentalFeature/enablement/set` accepts the current Rust-supported process-local feature allow-list (`apps`, `memories`, `plugins`, `remote_control`, `tool_search`, `tool_suggest`, and `tool_call_mcp_elicitation`). Direct root enables of under-development feature keys print the Rust-shaped unstable warning, while profile-scoped enables stay quiet; root disables of direct default-off feature keys clear the key instead of pinning `false`. App-server thread startup now emits Rust-shaped `warning` notifications for config-enabled under-development features, honors `suppress_unstable_features_warning = true`, and emits `deprecationNotice` notifications for legacy feature keys in config. Managed requirements remain planned. |
...
| `mcp` | partial | Supports `codex-zig mcp list|get|add|remove` for global `$CODEX_HOME/config.toml` stdio and streamable HTTP server entries, with `mcp add NAME COMMAND...` accepting stdio commands without requiring `--` and preserving option-looking tokens after the command starts, `mcp get --json NAME` accepting options before the server name, and `mcp get|remove|login|logout -- NAME` addressing option-looking server names, including Rust-shaped human `mcp list` stdio/http tables with masked env and stdio `cwd` columns, JSON output for list/get with streamable HTTP `http_headers` and `env_http_headers`, stdio `cwd`, `mcp get` masked human display for static headers, `mcp list` auth-status reporting for stdio, bearer-token, file-backed OAuth, macOS keychain-backed OAuth, and HTTP OAuth-discovery not-logged-in servers, and enabled active-version local plugin-cache `.mcp.json` entries when plugins are enabled, including plugin-relative stdio `cwd` resolution. `mcp add --url` now probes streamable HTTP OAuth metadata and starts the Rust-shaped OAuth login flow after writing the server entry when the server advertises OAuth support. Runtime stdio and streamable HTTP MCP tool discovery and `tools/call` execution work through the shared Responses session loop for configured servers and enabled local plugin-cache `.mcp.json` servers, model-facing stdio and streamable HTTP MCP `list_mcp_resources`, `list_mcp_resource_templates`, and `read_mcp_resource` calls work through the shared session loop for configured servers and enabled local plugin-cache `.mcp.json` servers, streamable HTTP MCP clients include configured static/env HTTP headers, file-backed OAuth credentials, macOS keychain-backed OAuth credentials in `auto`/`keyring` store modes, reuse `Mcp-Session-Id` response headers after initialize, can receive JSON-RPC responses from GET SSE streams after accepted POSTs, and send best-effort session teardown requests, app-server `mcpServerStatus/list` includes raw tool, resource, and resource-template snapshots for configured stdio/streamable HTTP servers and enabled local plugin-cache streamable HTTP servers, app-server `mcpServer/resource/read` can read text/blob resources from configured stdio/streamable HTTP MCP servers and enabled local plugin-cache stdio/streamable HTTP MCP servers without a thread or after validating an already-loaded `threadId`, app-server `mcpServer/tool/call` can call config-backed and enabled local plugin-cache stdio and streamable HTTP MCP tools for already-loaded threads with thread-id `_meta` injection, object/non-object argument pass-through, and stdio plus streamable HTTP `elicitation/create` bridging through `mcpServer/elicitation/request`, stdio, Unix-socket, and websocket app-server `mcpServer/oauth/login` returns an authorization URL, completes the PKCE callback flow in a worker, stores credentials, and emits `mcpServer/oauthLogin/completed`, `codex-zig mcp login` performs file-backed streamable HTTP OAuth browser login with metadata discovery, dynamic client registration, PKCE callback handling, token exchange, retries provider-rejected discovered scopes once without scopes, and Rust-compatible fallback credential writes, and `codex-zig mcp logout` removes matching file-backed OAuth credentials plus macOS keychain-backed OAuth credentials for configured streamable HTTP servers. `codex-zig mcp-server` exposes a stdio MCP server with `initialize`, `ping`, Rust-shaped `tools/list` schemas, and `tools/call` for `codex` plus in-process `codex-reply` thread continuation, including per-call `model`, `cwd`, `approval-policy`, `sandbox`, `base-instructions`, `developer-instructions`, `compact-prompt`, and supported `config` object override handling for fields such as `profile`, `instructions`, `developer_instructions`, and `compact_prompt`. Cross-platform keyring backends, true thread-owned MCP runtime reuse, persistent streamable HTTP server notification streams, and deeper MCP server cancellation/progress parity are planned. |
| `plugin` / `plugin marketplace` | partial | Supports `codex-zig plugin add`, `plugin list`, and `plugin remove` for configured local/git-backed marketplace snapshots and Rust-compatible personal HOME marketplace roots, including `PLUGIN@MARKETPLACE` and `--marketplace` selectors, Rust-shaped marketplace/plugin table output, local plugin cache installation, `$CODEX_HOME/config.toml` enablement, local cache/config removal, and Rust-visible root and local `-c/--config`, `--enable`, and `--disable` option parsing before or after plugin subcommands, including runtime `features.plugins` gate overrides plus marketplace-source and plugin enabled-state config overrides on list/add surfaces. Supports `codex-zig plugin marketplace add` for local marketplace directories and git marketplace sources, including GitHub shorthand normalization, `--ref`, sparse paths, manifest validation, `$CODEX_HOME/.tmp/marketplaces` installs, `$CODEX_HOME/config.toml` mutation, already-added reporting, and Rust-visible local config/feature options across marketplace subcommands, including runtime `features.plugins` gate overrides and config-overridden marketplace roots on `list`/`upgrade`; supports `list` for configured and home marketplace roots, including empty marketplace manifests and configured load failures; supports `upgrade [NAME]` for configured Git marketplaces with remote revision checks, installed-root replacement, `last_revision` config updates, `.codex-marketplace-install.json` metadata, already-up-to-date reporting, and per-marketplace failure reporting; supports `remove` for configured marketplace tables or installed cache roots. App-server `marketplace/add`, `marketplace/remove`, and `marketplace/upgrade` cover the same local and git-backed marketplace config mutation behavior, and app-server `plugin/list` lists configured local marketplace roots, local repo/home marketplace manifests, local and Rust-normalized git plugin sources, manifest metadata, installed/enabled state from `$CODEX_HOME`, feature gating, invalid marketplace load errors, absolute-cwd validation, and remote `chatgpt-global`, `workspace-directory`, and `shared-with-me` catalogs from the configured ChatGPT backend with installed/enabled state, pagination, installed-only global entries, default remote feature gating, explicit marketplace-kind handling, and auth/error fallback to local-only results. App-server `plugin/list` also returns Rust-shaped `featuredPluginIds` from `/plugins/featured?platform=...` when a visible `openai-curated` local marketplace is present, using the session-source product mapping and no-auth fallback. App-server `plugin/read` reads marketplace-backed local plugin summary metadata, manifest descriptions, skills with config-driven enabled state, command hook summaries, app ids, and MCP server names, and reads remote plugin details from the configured ChatGPT backend with catalog metadata, remote source summaries, installed/enabled state, disabled remote skills, basic app-id summaries, and workspace share context. App-server `plugin/skill/read` validates remote marketplace/plugin/skill params, requires ChatGPT-style auth, fetches remote skill Markdown from the configured ChatGPT backend, and returns Rust-shaped `contents`. App-server `plugin/share/save` archives local workspace plugin shares, requests upload URLs, uploads gzip tar payloads, finalizes shares with discoverability and targets, and records local path mappings; `plugin/share/list` lists created workspace shares with installed state and local path mappings; `plugin/share/updateTargets` updates share targets and returns Rust-shaped principals; `plugin/share/checkout` checks out shared workspace plugin bundles into the personal HOME plugin root, updates the personal marketplace manifest, records remote-to-local share mappings, preserves existing mapped local edits on repeat checkout, and returns Rust-shaped checkout metadata; and `plugin/share/delete` deletes remote shares plus local path mappings. App-server local `plugin/install` installs local and git-backed marketplace sources, including GitHub shorthand, relative git URLs, trimmed selectors, normalized subdirs, and `git-subdir` sparse checkouts, into the plugin cache, persists enabled config entries, and returns Rust-shaped auth-policy responses. App-server remote `plugin/install` fetches detail with bundle download URLs, rejects disabled or unavailable plugins, downloads gzip tar bundles over HTTPS or explicit loopback test URLs, rejects unsafe archive paths and symlinks, writes versioned cache entries, posts the ChatGPT backend install mutation, and returns Rust-shaped auth-policy responses. App-server local `plugin/uninstall` validates Rust-shaped local plugin ids, removes the local plugin cache root, clears the matching config table and child tables, and is idempotent. App-server remote `plugin/uninstall` fetches detail for the cache namespace, posts the ChatGPT backend uninstall mutation, validates disabled mutation responses, and removes current plus legacy local cache roots. Full marketplace update/cache parity, remote catalog cache synchronization, full app auth lookups, and full plugin cache parity are planned. |
...
| `apply` ChatGPT task diffs | partial | Supports `codex-zig apply TASK_ID` and alias `codex-zig a TASK_ID`, accepts `-c/--config features.*` plus `--enable/--disable` feature controls with Rust-compatible help preflight, loads backend ChatGPT or agent-identity auth, fetches `/wham/tasks/TASK_ID`, extracts the first PR `output_diff.diff`, and applies it in the current repository root after a `git apply --check --3way` preflight. Failed preflights report that no files were changed before returning an error, and `git apply` stdout/stderr is parsed into applied, skipped, and conflict path groups for diagnostic counts and conflict/skipped path summaries. The shared git-apply helper supports reverse application and reverse preflight, including Rust-compatible best-effort staging of existing paths before non-preflight reverse applies, literal pathspec staging, deletion-restore path exclusion to avoid mutating conflicting user files, conservative ambiguous rename/copy path handling, index restore after failed staged reverse applies, and temporary-index staging for reverse preflights. `zig build e2e` verifies success, alias success, and conflicting-preflight paths against a mock ChatGPT task backend and temporary git repositories; `zig build test` verifies reverse apply/preflight helper behavior against a temporary git repository. Deeper `codex_git_utils` fixture parity is planned. |
| `exec` non-interactive mode | partial | Supports `codex-zig exec` and visible Rust-compatible alias `codex-zig e` with `[--auto-approve] [--dangerously-bypass-approvals-and-sandbox] [--dangerously-bypass-hook-trust] [--yolo] [--ephemeral] [--skip-git-repo-check] [--ignore-user-config] [--ignore-rules] [-c key=value] [--strict-config] [--enable FEATURE] [--disable FEATURE] [--color MODE] [-m MODEL] [--oss] [--local-provider NAME] [-p PROFILE] [-a MODE] [-s MODE] [-C DIR] [--add-dir DIR] [--json] [-o FILE] [--output-last-message FILE] [--output-schema FILE] [-i IMAGE...] [PROMPT|-]`, Rust-compatible equals forms for long options including `--approval-policy=MODE`, `--output-last-message=FILE`, `--enable=FEATURE`, `--disable=FEATURE`, and `--profile=PROFILE`, variadic image attachment collection that stops at the next option or `--`, exec-local feature overrides are threaded into request construction and `--disable shell_tool` removes shell tools from the model-visible tool list, root exec prompt reads from piped stdin when no prompt is provided, prompt-plus-piped-stdin root exec appends stdin in Rust-compatible `<stdin>` context markup, trusted `SessionStart` and `UserPromptSubmit` command hooks run before exec/review turns and pass accepted hook context as developer messages, `--dangerously-bypass-hook-trust` includes enabled untrusted/modified hooks for the invocation, non-interactive exec and `exec review` refuse non-Git working directories unless `--skip-git-repo-check` or dangerous bypass is set, dangerous bypass conflicts with explicit approval-policy flags, hidden Rust-compatible `--full-auto` migration warning, `codex-zig exec resume [--all] [last|ID|PATH] PROMPT` using the shared session/tool loop with exec options accepted after `resume`, and `codex-zig exec review ...` dispatch to the shared review command after applying exec-level `--cd` plus pre- or post-`review` `-c/--config`, `--strict-config`, `--enable/--disable`, `-m/--model`, `--json`, `-o/--output-last-message`, `--ephemeral`, `--ignore-user-config`, `--ignore-rules`, `--skip-git-repo-check`, `--profile`, and dangerous-bypass controls. Full event parity, image resizing/normalization, and additional Rust exec flags are planned. |
| `exec` version flags | covered | `codex-zig exec -V` and `codex-zig exec --version` print `codex-cli-exec 0.0.1` on stdout, the exec help advertises the version flags, and the first help/version flag wins for mixed `--version --help` or `--help --version` arguments. |
| `exec` help command | covered | `codex-zig exec help`, `codex-zig exec help resume`, `codex-zig exec help review`, `codex-zig exec help help`, `codex-zig exec resume --help`, and `codex-zig exec review --help` render local help without entering the model-backed exec flow. |
| `review` non-interactive mode | partial | Supports `codex-zig review --uncommitted`, `codex-zig review --base BRANCH`, `codex-zig review --commit SHA [--title TITLE]`, custom review instructions from argv or `-` stdin, local `-c/--config` plus `--enable/--disable` review overrides, trusted Git-repository checks, top-level `review_model` selection for review requests, Rust review-rubric instructions, structured review JSON schema requests, structured review output rendering, and Rust-shaped `codex-zig exec review ...` routing through the shared Responses turn path with shared exec JSONL and last-message output support. Full Rust review event parity is planned. |
| `debug` | partial | Supports `codex-zig debug prompt-input [-i FILE...] [PROMPT]` to render the model-visible Responses input list without making an API call, including variadic local image attachments, `codex-zig debug models [--bundled]` for a Zig-native model catalog snapshot, and `codex-zig debug clear-memories` to clear `$CODEX_HOME/memories`, `$CODEX_HOME/memories_extensions`, `stage1_outputs`, and memory pipeline `jobs` rows while preserving root directories, symlink-root protections, and unrelated state-db jobs. Hidden `debug trace-reduce [-o FILE] TRACE_BUNDLE` writes Rust-shaped reduced `state.json` for stable rollout lifecycle, thread lifecycle, Codex turn lifecycle, inference lifecycle metadata, and raw payload refs from local trace bundles. `debug app-server send-message-v2 USER_MESSAGE` now spawns the local app-server over stdio, initializes the experimental client, starts a thread, sends a `turn/start` text message, prints JSON-RPC responses and notifications, and waits for the matching `turn/completed` notification. Online model refresh, full Rust app-server debug-test client option parity, full rollout conversation/tool/code-cell/compaction/agent trace reduction, and byte-for-byte reducer parity are planned. |
| `sandbox` macOS seatbelt | partial | Parses `read-only`, `workspace-write`, and `danger-full-access`; shell processes run through `/usr/bin/sandbox-exec` on macOS for read-only/workspace-write, `--add-dir` expands workspace-write shell writable roots, sandboxed CLI/tool/app-server command children receive `CODEX_SANDBOX=seatbelt`, and `codex-zig sandbox macos|seatbelt [OPTIONS] -- COMMAND` exposes the same Zig seatbelt wrapper from the CLI. Sandbox root and platform help now follow the Rust command structure, including `sandbox help <platform>` and `help sandbox <platform>` routing, platform-specific Linux/Windows help, and Rust-compatible `-c/--config` plus `--enable/--disable` acceptance before or after the platform subcommand. Rust-compatible `sandbox linux|landlock` and `sandbox windows` platform subcommands are recognized at parse time and return explicit unsupported-platform errors in the current port; sandbox `--full-auto` is rejected as a removed option. Rust permission-profile sandbox flags are parsed and validated, including `-C/--cd` and `--include-managed-config` requiring `--permissions-profile`; the built-in `:read-only`, `:workspace`, and `:danger-full-access` profiles are applied to macOS sandbox construction from the CLI; and custom `[permissions]` profiles that lower to the current root-read/workspace-write seatbelt model are applied, including project-root and absolute writable roots, concrete absolute/project-root read-deny entries, deny-read glob entries with original and canonicalized static-prefix regex rules, plus full network enable/disable. The shared session/tool path and app-server loaded-thread permission profiles now pass deny-read globs into command sandboxing, app-server command permission profiles can lower non-root read-only allowlists through a Rust-style closed-by-default profile with explicit `:minimal` platform-default gating and parent-deny carveouts, and in-process `apply_patch` uses conservative preflight checks for read-denied roots and globs instead of process sandboxing. The macOS-only Rust `--allow-unix-socket` flag is applied to Seatbelt profiles as AF_UNIX bind/connect allowlist rules rooted at normalized `UNIX_SOCKET_PATH_N` params, and `--log-denials` starts a command-scoped macOS log stream and prints a Rust-shaped post-command sandbox-denial summary. Full custom profile filesystem enforcement for narrow read allowlists, restricted custom-profile network allow-list policy, managed requirements enforcement, network proxy policy, and PID-filtered recursive descendant denial tracking parity are still planned. |
| `resume` / `fork` / session store | partial | Writes Zig-native JSONL transcripts under `$CODEX_HOME/sessions/zig/`, including persisted message image URLs/details that are restored onto their original history message and replayed as `input_image` content on later turns, starts `codex-zig resume` and `codex-zig fork` with numbered pickers, supports `resume --last`, `resume --all`, `resume --include-non-interactive`, `fork --last`, `fork --all`, `resume|fork [ID|PATH|last] [PROMPT]`, `remote-fork CODE` imports Rust-compatible protocol-version-1 local HTTP claim bundles into `$CODEX_HOME/sessions/remote-forks/` and starts a TUI fork from the imported thread ID, Rust-compatible interactive overrides after `resume` / `fork` / `remote-fork` such as model, profile, cwd, images, approval policy, sandbox mode, OSS provider, remote flags, local remote-control flags, and no-alt-screen, and `codex-zig exec resume --all ...`, lists saved sessions through `codex-zig sessions [N]` and `/sessions [n]`, supports local `codex-zig archive`, `codex-zig unarchive`, and `codex-zig delete` for saved rollout files with Rust-shaped UUID/name resolution, `delete --force UUID`, non-tty confirmation refusal, and missing-target errors, and supports `/resume` and `/fork` inside the TUI. The shared session loader also accepts basic Rust rollout JSONL envelopes under `$CODEX_HOME/sessions/YYYY/MM/DD/` and imported remote-fork rollouts for app-server resume/fork. Full Rust rollout browsing, state-db metadata, app-server fork metadata, cwd filtering, non-interactive session filtering, and rich browsing UI are planned. |
| app server / desktop app / cloud tasks | partial | Supports `codex-zig app [PATH] [--download-url URL]` on macOS to open an existing Codex Desktop app or download/install the DMG when missing, supports `codex-zig app-server [--listen URL] [--session-source SOURCE]`, Rust-compatible parsing for `stdio://`, `off`, `unix://`, `unix://PATH`, and `ws://IP:PORT`, accepts `--analytics-default-enabled`, parses Rust-compatible `--session-source SOURCE` with default `vscode` and app-server/MCP aliases, parses and validates Rust-compatible websocket auth flags (`--ws-auth`, `--ws-token-file`, `--ws-token-sha256`, `--ws-shared-secret-file`, `--ws-issuer`, `--ws-audience`, `--ws-max-clock-skew-seconds`), runs JSON-RPC over stdio, Unix sockets, or websocket text frames with `initialize`, `memory/reset`, `thread/start` in-memory loaded-thread creation, `thread/resume` Zig session JSONL, basic Rust rollout JSONL, and in-memory history loading, `thread/fork` in-memory loaded-thread plus Zig-native and basic Rust rollout transcript-path forking, `thread/loaded/list` stateful loaded-thread responses, `thread/read`, `thread/list`, and `thread/turns/list` loaded-thread reads, thread metadata/lifecycle RPCs (`thread/archive`, `thread/unarchive`, `thread/rollback`, `thread/inject_items`, `thread/name/set`, `thread/memoryMode/set`, and `thread/metadata/update`), `thread/goal/set|get|clear`, `thread/approveGuardianDeniedAction`, `thread/unsubscribe` connection-local subscribed/unsubscribed/not-subscribed/not-loaded responses, and `thread/compact/start`, `thread/shellCommand`, `thread/backgroundTerminals/clean|list|terminate`, `thread/increment_elicitation`, and `thread/decrement_elicitation` loaded-thread validation plus implemented loaded-thread compaction, shell-command, background-terminal control, and elicitation-counter behavior, app-server filesystem `fs/readFile`, `fs/writeFile`, `fs/createDirectory`, `fs/getMetadata`, `fs/readDirectory`, `fs/remove`, and `fs/copy`, model catalog `model/list` over bundled picker presets, enabled local plugin app catalog `app/list`, collaboration mode presets, provider capabilities `modelProvider/capabilities/read`, account state `account/read`, legacy remote diff `gitDiffToRemote`, legacy local file search `fuzzyFileSearch`, legacy auth status `getAuthStatus`, legacy `getConversationSummary` summary lookups for loaded threads and local rollout paths, API-key and external ChatGPT-token login through `account/login/start`, login cancellation through `account/login/cancel`, account rate-limit fetches through `account/rateLimits/read`, account token-usage fetches through `account/usage/read`, workspace-message fetches through `account/workspaceMessages/read`, rate-limit reset-credit consumption through `account/rateLimitResetCredit/consume`, workspace-owner nudge emails through `account/sendAddCreditsNudgeEmail`, account removal `account/logout` with `account/updated` notification, partial config reads through `config/read`, config writes through `config/value/write`, multi-edit config writes through `config/batchWrite`, system `requirements.toml` core allow-list, feature, hooks, residency, and network fields and legacy managed-config requirements through `configRequirements/read`, external-agent config detection/import through `externalAgentConfig/detect` and `externalAgentConfig/import`, experimental feature catalog `experimentalFeature/list`, process-local runtime feature enablement patches through `experimentalFeature/enablement/set`, local skill discovery through `skills/list`, skill enablement writes through `skills/config/write`, connection-local skill invalidation through `skills/changed`, accepted MCP config reload requests through `config/mcpServer/reload`, MCP server status listing through `mcpServerStatus/list`, filesystem watch registration through `fs/watch`, watch removal through `fs/unwatch`, in-process and request-boundary external mutation notifications through `fs/changed`, buffered one-off command execution through `command/exec`, synchronous experimental process execution through `process/spawn` with `process/outputDelta` and `process/exited` notifications plus stdio, Unix-socket, and websocket async `process/spawn` streamed-output, stdin, and TTY sessions with active `process/writeStdin`, `process/kill`, and `process/resizePty`, local and remote marketplace listing through `plugin/list` including configured local marketplace roots, local and remote plugin detail reads through `plugin/read`, remote plugin skill detail reads through `plugin/skill/read`, remote plugin share save, listing, target updates, checkout, and deletion through `plugin/share/save|list|updateTargets|checkout|delete`, local and remote plugin installation through `plugin/install`, local plugin cache/config removal and remote plugin uninstall mutation plus cache cleanup through `plugin/uninstall`, local and git-backed `marketplace/add`, `marketplace/remove`, and `marketplace/upgrade` config/cache mutation, and remaining `plugin/*` not-implemented responses, plus JSON-RPC errors for unsupported request methods, supports `codex-zig app-server proxy [--sock SOCKET_PATH]`, hidden `codex-zig stdio-to-uds SOCKET_PATH` for stdio-to-Unix-socket JSON-RPC relay, `codex-zig app-server generate-ts -o DIR [--experimental]` for TypeScript bindings covering initialize, command exec, process RPC, and the implemented app-server request/response surface, `codex-zig app-server generate-json-schema -o DIR [--experimental]` for JSON-RPC envelope, command-exec, process RPC, and the implemented app-server request/response schemas, and hidden `codex-zig app-server generate-internal-json-schema -o DIR` for a minimal `RolloutLine.json` schema. `gitDiffToRemote` covers closest remote SHA selection for ordinary origin-backed repos, tracked working-tree diffs, untracked file diffs, invalid params, and missing-repo errors; `fuzzyFileSearch` covers one-shot local file/directory subsequence matching, Rust-compatible ASCII path scoring/order/indices for ordinary paths, empty query results, cancellation token shape validation, invalid params, JSON field compatibility, composed/decomposed Latin accent folding with Rust-compatible character indices for UTF-8 paths, and connection-local `fuzzyFileSearch/sessionStart|sessionUpdate|sessionStop` notifications while broader Unicode normalization, request cancellation, and true async streaming are still planned; `thread/start` covers in-memory loaded-thread creation, Rust-shaped start responses, ephemeral starts, persistent rollout-path assignment, generated bindings, JSON schemas, and start/fork `thread/started` notifications with opt-out filtering while Rust state-db rollout metadata and full async turn execution parity remain planned; `thread/loaded/list` covers nullable or object params, cursor/limit shape validation, and stateful Rust-shaped `data` / `nextCursor` response fields; `thread/unsubscribe` covers `threadId` UUID validation, Rust-shaped `notLoaded` responses for unknown threads, connection-local `unsubscribed` responses for threads subscribed through start/resume/fork, and `notSubscribed` for repeat or never-subscribed loaded threads; `thread/compact/start`, `thread/shellCommand`, `thread/backgroundTerminals/clean|list|terminate`, `thread/increment_elicitation`, and `thread/decrement_elicitation` cover `threadId` UUID validation, Rust-shaped `thread not found` responses for unloaded threads, shell-command empty command validation, background-terminal cursor/process-id validation, and implemented loaded-thread compaction, shell-command, background-terminal control, and elicitation-counter behavior; `skills/list` covers Rust-shaped params, default cwd fallback, repo/user/per-cwd extra/enabled local plugin skill roots, plugin skill name prefixing from plugin manifests, SKILL.md frontmatter name/description/shortDescription extraction, `agents/openai.yaml` interface and tool-dependency metadata, config-driven `enabled` state by name or absolute path, connection-local cwd result caching until `forceReload`, connection-local `skills/changed` cache invalidations for app-server skill-root mutations and skills config writes, invalid skill-file errors, and malformed-param errors while policy metadata, kernel-backed external skill watching, debouncing, and full multi-layer cache parity are still planned; `skills/config/write` covers name/path selector validation, disabled `[[skills.config]]` persistence in `$CODEX_HOME/config.toml`, enabled-removal toggles, and Rust-shaped `effectiveEnabled` responses; `mcpServerStatus/list` covers configured plus enabled local plugin-cache `.mcp.json` server names, sorting, pagination, `detail` validation, `resourceTemplates` casing, raw stdio and streamable HTTP tool/resource snapshots, empty resource/resource-template snapshots in `toolsAndAuthOnly` mode, and bearer-token auth status from environment variables while strict live refresh, live MCP startup, OAuth login/logout, and status notifications are still planned; `fs/watch`/`fs/unwatch` covers connection-local watch IDs, duplicate watch rejection, absolute-path validation, Rust-shaped `fs/changed` notifications for filesystem mutations made through the same app-server connection, request-boundary external change polling after direct filesystem mutations, and unwatch cleanup while kernel-backed async change detection and debouncing are still planned; `command/exec` covers buffered argv execution with stdout/stderr capture, stdio/Unix-socket/websocket `streamStdoutStderr` output-delta notifications, `streamStdin` active write sessions with client-supplied `processId`, and macOS `tty` write/resize sessions, cwd and environment overrides, nonzero exit responses, output caps, timeouts, default or explicit `dangerFullAccess`/`readOnly`/`workspaceWrite` sandbox policy handling, supported `externalSandbox` policy handling, supported `permissionProfile` mappings for disabled, root-read-only, managed-unrestricted, project-roots workspace-write, absolute writable roots, and external profiles, plus explicit not-implemented responses for unsupported permission-profile entries while full command-exec lifecycle parity is still planned; `process/*` covers synchronous non-PTY `process/spawn`, output/exited notifications, output caps, timeouts, cwd/env validation, stdio/Unix-socket/websocket async streamed-output, stdin, and TTY process sessions, active write/kill/resize follow-ups, and duplicate active handles while broader async process lifecycle parity remains planned; `model/list` covers bundled picker ordering, fresh compatible-client-version `models_cache.json` loading with Rust TTL/version fallback behavior, OnlineIfUncached remote refresh for ChatGPT/agent-identity/provider-auth-command contexts with ETag cache persistence, X-Models-Etag turn-response TTL renewal for matching caches, cache entries overlaid onto the bundled catalog, cache priority sorting, hidden filtering via `includeHidden`, pagination cursors, default markers, upgrade metadata, availability nux, reasoning efforts, modality defaults, speed-tier arrays, service tiers, personality support detection, and configured-model isolation while broader in-memory forced-refresh manager parity remains planned; `app/list` covers empty no-manifest pages, enabled local plugin `.app.json` rows from plugin cache, home, loaded-thread cwd marketplaces, authenticated ChatGPT connector directory pages, accessible connector discovery from live `codex_apps` MCP metadata, simple app enabled flags, pagination cursors, optional params, and loaded-thread validation while broader cache invalidation and workspace-gated availability remain planned; `collaborationMode/list` covers the built-in Plan and Default presets with Rust-shaped `mode`, `model`, and `reasoning_effort` fields; `account/read` covers no-auth, API-key, stored ChatGPT id-token metadata, external ChatGPT-token metadata, Bedrock, custom no-auth provider, and local-OSS states; `getAuthStatus` covers no-auth, API-key token inclusion/omission, stored ChatGPT access tokens, external ChatGPT auth tokens, agent identity token omission, custom no-auth providers, local-OSS, nullable params, and invalid param errors; `account/login/start` covers API-key and external ChatGPT-token login with `account/login/completed` and `account/updated` notifications, plus browser ChatGPT login start with local callback server binding, Rust-shaped `loginId` / `authUrl`, forced-workspace URL constraints, active-login replacement, callback completion, token persistence, account notifications, cancellable failure notification, success redirects through `/success`, and callback error/missing-code HTML pages, and device-code ChatGPT login start, completion, token persistence, account notifications, and cancellation; `account/login/cancel` validates login IDs, cancels active browser and device-code ChatGPT login IDs with `canceled`, returns `notFound` for missing or already-completed login IDs, and browser/device-code background completions clear matching active-login state; `account/rateLimits/read` covers ChatGPT/agent-identity backend fetching, response mapping, no-auth/API-key rejection, and invalid params; `account/usage/read` covers ChatGPT backend profile fetching, response casing, nullable/daily token usage stats, permissive nullable/object params, and no-auth/API-key rejection; `account/workspaceMessages/read` covers ChatGPT backend message fetching, disabled 404 handling, headline/announcement/unknown type mapping, RFC3339 epoch timestamp conversion, permissive nullable/object params, and no-auth/API-key rejection; `account/rateLimitResetCredit/consume` covers idempotency/credit request mapping, reset/nothingToReset/noCredit/alreadyRedeemed outcome mapping, Rust-shaped param validation, and no-auth/API-key rejection; `account/sendAddCreditsNudgeEmail` covers ChatGPT/agent-identity backend posting, `sent` and `cooldown_active` status mapping, no-auth/API-key rejection, invalid params, and backend-failure JSON-RPC errors. `config/read` currently returns the Zig port's effective core config fields, effective feature flags including profile-scoped feature overrides, Rust-shaped user `config.toml` origins for supported scalar fields, trusted project-stack `.codex/config.toml` origins/layers for supported scalar fields (`model`, `review_model`, `model_context_window`, `model_auto_compact_token_limit`, `approval_policy`, `sandbox_mode`, `web_search`, `model_reasoning_effort`, `model_verbosity`, and `service_tier`) and supported `tools` and `apps` fields between a trusted project root and an absolute child `cwd`, legacy managed-config scalar/tool/app/sandbox origins and layers ahead of lower-precedence layers, required empty user/system layers, system config scalar/tool/app/sandbox origins and layers below user/project layers, and project-before-user layers when `includeLayers` is true; `config/value/write` covers supported TOML key paths with scalar, array, inline-object, existing table-object merge/replace, and null-clearing values, `replace`/`upsert` validation, optional `filePath` limited to the user `config.toml`, SHA-256 `expectedVersion` conflicts, and Rust-shaped write responses; `config/batchWrite` covers multiple in-memory edits persisted with one file write plus existing table-object merge/replace, optional `filePath` limited to the user `config.toml`, SHA-256 `expectedVersion` conflicts, `reloadUserConfig` shape validation, inline-object values used by hook state writes, null-clearing values, and Rust-shaped write responses. Hot reload notifications for loaded threads, full override metadata, full config-manager validation, system/full-project/managed layer fidelity, nested-field origins, full cloud/MDM requirements, app/plugin requirement fields, and managed requirements enforcement are still planned; `configRequirements/read` reads system `requirements.toml` core allow-list fields (`allowed_approval_policies`, `allowed_approvals_reviewers`, `allowed_sandbox_modes`, `allowed_web_search_modes`), top-level `enforce_residency`, `[hooks]` managed directories and command/prompt/agent matcher groups, basic `[experimental_network]` scalar booleans and ports, canonical domain and Unix-socket permission maps, and legacy domain/socket allow-list arrays, plus `[features]` and `[feature_requirements]` booleans and then lets Rust-compatible legacy `managed_config.toml` fields `approval_policy`, `approvals_reviewer`, and `sandbox_mode` fill unset managed allow-list responses; it returns `requirements: null` when none are configured. `memory/reset` uses the same directory-clearing safety rules and SQLite row cleanup as `debug clear-memories`. `unix://` listens on `CODEX_HOME/app-server-control/app-server-control.sock`; `unix://PATH` listens on the requested socket path. `zig build e2e` launches the rebuilt binary, verifies profile-scoped features CLI and app-server behavior, and verifies stdio, memory reset, thread-start loaded-thread creation, disk-backed Zig and basic Rust rollout plus in-memory-history thread-resume loading, in-memory plus Zig-native and basic Rust rollout path thread-fork creation, stateful loaded-thread listing/read/turns-list/unsubscribe, and compact/shell-command/background-terminal clean/list/terminate/elicitation-counter empty-runtime behavior, local and git-backed marketplace add/list/repeat/upgrade/remove RPC behavior, plugin-list marketplace discovery, local plugin-read details, remote plugin-list catalogs, remote plugin-read details, remote plugin-skill reads, remote plugin-share save/list/update/checkout/delete behavior, local, pinned git-subdir, and remote plugin-install cache writes, local plugin-install config writes, local plugin-uninstall cache/config cleanup, and remote plugin-uninstall cache cleanup against a mock backend, skills-list discovery/config writes/change notifications, MCP server status listing including enabled local plugin-cache `.mcp.json` entries and streamable HTTP inventory snapshots, MCP resource/tool success and JSON-RPC error forwarding for stdio, streamable HTTP POST, and streamable HTTP GET SSE responses, filesystem watch notifications including external direct-file mutation polling, filesystem RPC behavior against a temporary directory, buffered command-exec RPC behavior, process RPC behavior, model catalog, app-list enabled local plugin app catalog, collaboration-mode-list, git-diff-to-remote, fuzzy-file-search, account-read, get-auth-status, account-logout, account-login, account-login-cancel, account-login-browser-callback, account-login-device-code, account-rate-limits, account-usage, account-add-credits-nudge, config-read including trusted project scalar/tool/app layers, required empty user/system layers, system scalar/tool/app/sandbox config precedence, and legacy managed scalar/tool/app/sandbox overrides, config-value-write including table-object merge/replace, config-batchWrite including table-object merge/replace, external-agent config detect/import migration behavior including local plugin imports, and config-requirements RPC behavior against temporary config homes including system `requirements.toml` precedence, feature requirements, hooks requirements, residency requirements, network domain/socket requirements, and legacy managed config requirements, experimental feature list and runtime enablement behavior against temporary config homes, explicit Unix socket, default Unix socket, app-server proxy, stdio-to-uds, websocket transport, TypeScript and JSON schema generation, and app-server flag-compatibility handshakes. Kernel-backed asynchronous filesystem watch notifications, broader Unicode-normalized fuzzy scoring, request cancellation, and true async fuzzy file-search streaming, remaining account login/backend edge-case parity, remaining remote thread-store lifecycle APIs and active-turn tracking, full connector app-list parity, full config layer/origin fidelity, full external-agent config migration parity, managed config requirements enforcement, full live MCP status parity, broader in-memory forced model refresh parity, full TypeScript generation parity, full public schema parity, full internal schema parity, broader desktop app protocol parity, and richer Cloud picker and apply-diagnostic polish remains planned. |
...
`codex_apps` MCP tool metadata are implemented, while broader cache
invalidation and workspace-gated availability remain planned.
...
response, and applies local app disable pins to the effective `apps` object,
including pins that materialize disabled apps absent from user config.
App-server config writes now reject user/profile `[features]` values that
conflict with parsed feature requirements, and reject explicit top-level or
profile-scoped `approval_policy`, `approvals_reviewer`, `sandbox_mode`, and
...
requirements loading, broader plugin requirement enforcement beyond local
...
header redraw for `/clear`, in-session remote compaction through
...
relative roots resolved against the TUI cwd. Additional focused coverage verifies
remote TUI initial prompts and stdin prompts reject oversized text before
`turn/start`; stdin rejection drains oversized multibyte lines without exiting
and without sending the prompt to the app-server-backed Responses turn.

Additional remote app-server TUI config coverage: remote TUI `--profile`,
`-c profile=...`, `-c model_provider=...`, `-c openai_base_url=...`,
`-c chatgpt_base_url=...`, `--search`, `--oss`, and `--local-provider` are
accepted for start, resume, and fork flows and forwarded through thread
lifecycle `params.config`; overlay `--profile` stays local while `-c profile=...` is forwarded as `profile`, alongside `model_provider`, `openai_base_url`,
...
smoke that performs the websocket upgrade and JSON-RPC `initialize` handshake.

Additional local remote-control coverage: `--remote-control` now serves a
...
`apply`, and `diff`, parse command-specific options such as `--env`,
...
Cloud apply preflights report that no files were changed and include parsed
conflict/skipped path summaries before returning an error. `exec` creates Cloud
tasks from environment ids or case-insensitive environment labels,
argument/stdin prompts, branch overrides or current-branch
...
listeners, including listeners with connected idle clients, now exit cleanly
...
explicit per-thread overrides. The batch-write smoke now asserts model
provider, approval policy, and sandbox mode reloads on the loaded-thread fork
path, alongside service tier, approvals reviewer, and reasoning effort. The
hook runtime smoke also covers reloaded `hooks.state` trust and disable changes
on already-loaded threads.
...
Additional app-server config requirements coverage: `configRequirements/read`
now accepts Rust-valid `allowed_sandbox_modes = ["external-sandbox", ...]`
...
Additional app-server permission-profile coverage: `permissionProfile/list` is included in current TypeScript and JSON schema generation with cursor/limit/cwd params, Rust's built-in `:read-only`, `:workspace`, and `:danger-full-access` entries first, system/user/trusted-project/legacy-managed `[permissions.<id>]` summaries sorted by id with optional descriptions, layer precedence for duplicate profile ids, system or user project trust with user override, trusted project-local profile discovery when `cwd` is provided and bounded by Rust-style `project_root_markers` (default `.git`), linked-worktree main-root trust, child project profile precedence over parent project profiles, explicitly untrusted child project layers skipped, Rust-shaped pagination, and invalid-cursor errors. App-server `thread/start` and `turn/start` permission-profile selection accept Rust's profile-id string wire shape, resolve the same effective layer stack, and accept Rust's `:workspace_roots` filesystem key in supported root-read profiles while returning protocol-level `project_roots` entries until broader desktop protocol generation parity lands.
...
or authenticated remote directory pages are available, lists enabled local
plugin `.app.json` manifests from plugin-cache roots, `$CODEX_HOME`
marketplaces, and already-loaded thread cwd marketplaces as `AppInfo` rows,
loads authenticated ChatGPT connector directory pages from
`/connectors/directory/list?external_logos=true`, follows `nextToken`, reuses a
...
with Rust-shaped `nextCursor` values and invalid-cursor errors, and includes
...
Additional app-server conversation-summary coverage: legacy `getConversationSummary` reads summaries for already-loaded threads by `conversationId`, local `state_5.sqlite` rollout rows by `conversationId`, and local Zig/Rust rollout transcripts by `conversationId` or relative/absolute `rolloutPath`, returning Rust-shaped conversation ids, canonical paths, preview text, state-backed timestamps when available, model provider, cwd, CLI version, session source, and git metadata from state DB rows or transcripts. Generated TypeScript now includes the top-level session-source, conversation-summary, request, and response files plus `ClientRequest` / `ClientResponse` entries until full remote thread-store summary parity lands.

Additional app-server session-source coverage: `codex-zig app-server` now
accepts Rust-compatible `--session-source SOURCE`, defaults to `vscode`,
normalizes known startup aliases (`app-server`, `app_server`, and `appserver`)
to the app-server/MCP source, lowercases custom sources while preserving them
...
entries. `agents/openai.yaml` skill `policy.products` is also honored for
local `plugin/read` skill summaries and `skills/list` results. Remote
marketplace product-restriction query parity remains planned.
...
`plugin/share/delete`, `plugin/install`, `plugin/uninstall`, `review/start`,
`config/read`, `config/value/write`, `config/batchWrite`, and
`configRequirements/read`, plus
...
shape used by completed import notifications. Generated TypeScript and JSON
schemas also include current Rust-side `turn/moderationMetadata` and
`model/safetyBuffering/updated` notification payloads. No current user-facing
generated `ServerNotification` method-set gap is known; current Rust exposes
`rawResponseItem/completed`, which Zig already generates and emits, not the
stale `rawResponse/completed` name.
...
Additional app-server background-terminal control coverage:
`thread/backgroundTerminals/clean`, `thread/backgroundTerminals/list`, and
`thread/backgroundTerminals/terminate` now operate on model-facing PTY sessions
started by `exec_command` tool calls during `turn/start` for the loaded thread.
They require the Rust-shaped `experimentalApi` initialize capability and
preserve invalid and missing-thread errors when the capability is enabled.
`list` returns Rust-shaped `data` / `nextCursor` pages with `itemId`, string
`processId`, command, cwd, `osPid`, and null CPU/RSS metrics using the Rust
process-id cursor rule. `terminate` parses string `processId`, removes only the
matching thread-owned session, and returns Rust-shaped `terminated`. Full Rust
async active-process tracking, non-PTY terminal metadata, and live CPU/RSS
sampling remain planned.

Additional local TUI background-terminal lifecycle coverage: local TUI exit
now silently terminates any process-owned background terminal sessions, matching
Rust's shutdown cleanup for unified exec processes. The PTY smoke starts a
delayed model-requested terminal, exits with `/quit`, and verifies the delayed
process cannot write its survivor marker after the TUI exits.
...
Additional app-server turn-start coverage: `turn/start` now accepts text, URL-backed image, localImage, skill, and mention input for a loaded thread, including turns made only of structured skill/mention items. It preserves text input item arrays, `text_elements`, image URLs, local image paths, and skill/mention name/path pairs in user-message lifecycle notifications, runs the existing Responses turn loop with the thread transcript, roots model-requested shell tools and `apply_patch` edits in the loaded thread cwd when the tool call does not provide an explicit workdir, forwards URL-backed and readable local images as `input_image` request content, converts missing local images into model-visible error placeholders, injects readable local skill files as Rust-shaped `<skill>` user fragments, preserves structured mention targets as model-visible markers, injects Rust-shaped developer guidance for `plugin://...` mentions that resolve to enabled local plugins with visible skill, MCP, or app capabilities, updates loaded-thread preview/turns, persists persistent Zig transcripts, returns a Rust-shaped `TurnStartResponse`, and sends the response plus active `thread/status/changed` and `turn/started` notifications before the provider response completes for normal model turns. It emits text user-message `item/started` / `item/completed`, raw Responses output items as `rawResponseItem/completed`, final assistant-message `item/started`, `item/agentMessage/delta`, `item/completed`, and `turn/completed` notifications, and includes `turn/start`, `turn/started`, `turn/completed`, `turn/moderationMetadata`, `model/safetyBuffering/updated`, `item/started`, `item/completed`, `item/agentMessage/delta`, and the current `UserInput` union in generated TypeScript and JSON schemas. `turn/start` and `turn/steer` enforce Rust's 1048576-character text input limit across text items and return structured `input_too_large` JSON-RPC errors before starting or steering a turn. `turn/steer` and `turn/interrupt` are also parsed and generated: no-active-turn paths validate loaded-thread ids, steer input, `expectedTurnId`, and `turnId`, return Rust-shaped no-active-turn errors for loaded synchronous threads, and accept the Rust startup-interrupt empty `turnId` response. Regular turns now also accept `turn/steer` while a server-request approval, permission request, or `request_user_input` prompt is pending; accepted text, image, local-image, skill, and mention input is materialized into the same turn after the pending tool/request result and before the next Responses request. Loaded-thread `active` status now reports Rust-shaped `activeFlags` while these waits are outstanding: `waitingOnApproval` for command/file approval, permission, and MCP elicitation requests, and `waitingOnUserInput` for `request_user_input` prompts. The current Zig implementation still runs each turn synchronously after the early lifecycle notification and flattens model-visible turn input into the stored transcript; backend moderation/safety-buffering event emission, full Rust async streaming, app mention activation, provider-flight steering, chunked deltas and tool/reasoning item lifecycles, async interruption, and durable active-turn tracking remain planned.
...
model completion and before the turn lifecycle notifications, plus the idle state after turn completion,
...
Additional app-server thread rollback coverage: `thread/rollback` validates `threadId` and `numTurns`, preserves Rust-shaped `numTurns must be >= 1` rejection, returns Rust-shaped `thread not found` responses for unloaded threads, rejects active loaded threads while a turn is in progress, rejects ephemeral loaded threads without persisted rollout history, rolls back persistent loaded threads by trimming the last requested user-message turns from the in-memory transcript, refreshes loaded-thread preview/turns, appends Rust-shaped `thread_rolled_back` markers for persistent Zig transcripts, reconstructs simple Zig/Rust transcript history through append-only rollback markers when loading sessions, and is included in current TypeScript and JSON schema generation as an opaque-thread response until full thread schema parity lands. Pending-rollback tracking, complete compaction/context reconstruction, and full thread schema parity remain planned.
...
Additional app-server thread item-injection coverage: `thread/inject_items` validates `threadId` and `items` array shape, returns Rust-shaped `thread not found` responses for unloaded threads before response-item validation, appends supported raw message/function-call/function-call-output response items to already-loaded threads, joins nonblank text fragments from raw message content arrays, preserves injected message `input_image` content items in the loaded transcript and subsequent `turn/start` model requests, accepts string `function_call_output.output` values plus structured `input_text` / `input_image` output arrays, preserves structured function-call-output arrays in the loaded transcript, Zig session store, and subsequent `turn/start` model requests, persists the updated Zig transcript, includes injected items in subsequent `turn/start` model requests, and is included in current TypeScript and JSON schema generation as an opaque item-list request. Full raw response-item schema parity beyond message/function-call/function-call-output remains planned.
...
read and mutate the loaded-thread goal state, accept Rust-shaped
`token_budget` create params, restrict `update_goal` to `status = "complete"`,
return JSON goal payloads as function-call output, parse live Responses token
usage into `thread/tokenUsage/updated`, account loaded-thread goal token/time
usage from the active transcript baselines, persist Zig transcript goal fields,
save refreshed goal snapshots into local `state_5.sqlite` `thread_goals` rows
when present, restore saved transcript and local state-DB goals through
`thread/resume`, and emit `thread/goal/updated` with the active `turnId`.
Automatic goal continuation and remote goal-store parity remain planned.
...
Additional app-server thread delete coverage: `thread/delete` validates `threadId`, rejects ephemeral loaded threads with Rust's persisted-thread error, hard-deletes active or archived local rollout files for the requested thread plus state-DB spawned descendants in Rust's descendant-first notification order, accepts loaded persistent threads whose rollout file has not materialized yet, removes matching process-local loaded/subscription/unload state, deletes local `threads`, `thread_goals`, and `thread_spawn_edges` state rows when present, emits `thread/deleted`, returns the Rust-shaped missing-thread error for valid missing threads, and is included in current TypeScript and JSON schema generation along with `ThreadDeletedNotification`. Remote thread-store delete, compressed rollout deletion, and broader memory/retry-graph state cleanup remain planned.
...
Additional app-server thread turns-list coverage: `thread/turns/list` validates `threadId`, cursor, limit, sort direction, and `itemsView`, paginates transcript-derived turns for threads already loaded in the current app-server process, active saved Zig/Rust rollout files from `$CODEX_HOME/sessions`, archived saved Zig/Rust rollout files from `$CODEX_HOME/archived_sessions`, or local `state_5.sqlite` `threads.rollout_path` rows that point to readable rollout files using Rust-shaped descending-by-default ordering, JSON cursors, `nextCursor`, and `backwardsCursor`, defaults standalone page rendering to Rust's `itemsView: "summary"` behavior, honors explicit `itemsView: "notLoaded"`, `"summary"`, and `"full"` page rendering, rejects ephemeral loaded threads and persistent loaded threads before the first materialized turn with Rust-shaped invalid-request messages, and returns Rust-shaped `thread not loaded` responses for valid missing threads. Full state-DB metadata listing, remote thread-store listing, live active-turn merging, token-usage replay, full turn reconstruction, and complete turn schema parity remain planned.
...
Additional app-server goal coverage: `thread/goal/set`, `thread/goal/get`, and `thread/goal/clear` honor the `goals` feature gate, validate `threadId`, objective/status/token-budget params, reject ephemeral loaded threads, maintain loaded-thread goal state for already-loaded persistent threads, persist set/update/clear changes into Zig-native transcript metadata, operate on local `state_5.sqlite` `thread_goals` rows for unloaded state-DB-backed threads, restore saved transcript and local state-DB goals through `thread/resume`, emit `thread/goal/updated` and `thread/goal/cleared` notifications, return Rust-shaped `thread not found` responses for valid missing threads, and include generated TypeScript and JSON schemas for the goal requests, responses, `thread/goal/updated`, and `thread/goal/cleared`. Remote goal-store updates and complete thread/goal schema parity remain planned.
