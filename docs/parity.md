...
streamable HTTP elicitation and broader JSON Schema parity remain planned.
...
by `serverRequest/resolved`. True thread-owned MCP runtime reuse, persistent
streamable HTTP server notification streams, streamable HTTP elicitation, and
progress remain planned.
...
after accepted POSTs. Persistent streamable HTTP server notification streams,
streamable HTTP elicitation, and true thread-owned MCP runtime reuse remain
planned.
