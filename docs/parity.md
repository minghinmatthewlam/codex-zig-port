...
OAuth-discovery not-logged-in servers. Runtime use of keyring-backed login
credentials, retrying provider-rejected discovered scopes without scopes,
configured callback port/URL overrides, cross-platform keyring backends, and
full app-server OAuth login completion remain planned.
