...
| `resume` / `fork` / session store | partial | Writes Zig-native JSONL transcripts under `$CODEX_HOME/sessions/zig/`, starts `codex-zig resume` with a numbered picker, supports `codex-zig resume --last` / `codex-zig resume [ID|PATH|last]` and `codex-zig exec resume ...`, lists saved sessions through `codex-zig sessions [N]` and `/sessions [n]`, and supports `/resume` inside the TUI. Full Rust rollout path layout, metadata, fork semantics, and rich browsing UI are planned. |
