...
`streamStdoutStderr` output-delta notifications, stdio, Unix-socket, and
websocket app-server deferred command sessions with client-supplied
`processId`, active `command/exec/write` stdin streaming, macOS PTY-backed
`tty` command sessions with output deltas and active `command/exec/resize`, and
active `command/exec/terminate` handling,
...
roots. Fully deferred non-stream command responses, non-stdio PTY lifecycle
polish, and full Rust async command session parity remain planned. Inactive
...
async `process/spawn` sessions for `streamStdoutStderr` and `streamStdin`,
active `process/writeStdin`, duplicate active-handle rejection, active
`process/kill`, streamed process output deltas, and process-exit notifications;
stdio also covers macOS `tty` sessions with active `process/resizePty`.
Non-stdio PTY lifecycle routing and broader Rust process lifecycle polish remain
planned. Empty
