...
concrete absolute or special-path read-deny roots, and external profiles, with
command-cwd sandbox rooting for project roots.
...
seatbelt wrapper. Unknown, non-root-read-only, and read/write glob entries still
return explicit unsupported-profile errors.
