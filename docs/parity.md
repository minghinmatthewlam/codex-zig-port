...
the configured issuer, saves ChatGPT tokens, emits completion/account-update
notifications, and supports `account/login/cancel` with `canceled` plus
`account/login/completed` failure notification. Device-code ChatGPT login now
...
completion/account-update notifications, and supports cancellation. Active-login
cleanup after background completion and finer browser redirect/error-page parity
remain planned.
