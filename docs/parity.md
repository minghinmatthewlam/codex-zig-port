...
| `exec` non-interactive mode | partial | Supports `codex-zig exec` and visible Rust-compatible alias `codex-zig e` with `[--auto-approve] [--dangerously-bypass-approvals-and-sandbox] [--dangerously-bypass-hook-trust] [--yolo] [--ephemeral] [--skip-git-repo-check] [--ignore-user-config] [--ignore-rules] [-c key=value] [--strict-config] [--enable FEATURE] [--disable FEATURE] [--color MODE] [-m MODEL] [--oss] [--local-provider NAME] [-p PROFILE] [--profile-v2 PROFILE] [-a MODE] [-s MODE] [-C DIR] [--add-dir DIR] [--json] [-o FILE] [--output-last-message FILE] [--output-schema FILE] [-i IMAGE...] [PROMPT|-]`, Rust-compatible equals forms for long options including `--approval-policy=MODE`, `--output-last-message=FILE`, `--enable=FEATURE`, `--disable=FEATURE`, and `--profile-v2=PROFILE`, variadic image attachment collection that stops at the next option or `--`, exec-local feature overrides are threaded into request construction and `--disable shell_tool` removes shell tools from the model-visible tool list, root exec prompt reads from piped stdin when no prompt is provided, prompt-plus-piped-stdin root exec appends stdin in Rust-compatible `<stdin>` context markup, trusted `SessionStart` and `UserPromptSubmit` command hooks run before exec/review turns and pass accepted hook context as developer messages, `--dangerously-bypass-hook-trust` includes enabled untrusted/modified hooks for the invocation, non-interactive exec and `exec review` refuse non-Git working directories unless `--skip-git-repo-check` or dangerous bypass is set, dangerous bypass conflicts with explicit approval-policy flags, hidden Rust-compatible `--full-auto` migration warning, `codex-zig exec resume [--all] [last|ID|PATH] PROMPT` using the shared session/tool loop with exec options accepted after `resume`, and `codex-zig exec review ...` dispatch to the shared review command after applying exec-level `--cd` plus pre- or post-`review` `-c/--config`, `--strict-config`, `--enable/--disable`, `-m/--model`, `--json`, `-o/--output-last-message`, `--ephemeral`, `--ignore-user-config`, `--ignore-rules`, `--skip-git-repo-check`, `--profile-v2`, and dangerous-bypass controls. Full event parity, image resizing/normalization, and additional Rust exec flags are planned. |
...
`-c model_provider=...`, `-c openai_base_url=...`,
