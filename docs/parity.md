...
| --- | --- | --- |
| `cli` base interactive command | partial | First milestone launches `codex-zig` interactive loop and supports global `-m/--model`, `-p/--profile`, `-a/--ask-for-approval`, `-s/--sandbox`, and `--yolo` overrides. Full flag/subcommand parity is planned. |
...
| `exec` non-interactive mode | partial | Supports `codex-zig exec [--auto-approve] [--ephemeral] [-m MODEL] [-p PROFILE] [-a MODE] [-s MODE] [--json] [-o FILE] [PROMPT|-]` and `codex-zig exec resume [last|ID|PATH] PROMPT` using the shared session/tool loop. Review, schema output, and full event parity are planned. |
