...
support `/status`, `/sessions [N]`, `/resume [TARGET|last]`, and
`/fork [TARGET|last]` against the remote app-server thread APIs. The PTY TUI
E2E covers status printing, remote session listing, in-session remote fork, and
in-session remote resume while preserving the remote transcript history.
