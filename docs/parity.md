...
post-response `process/exited` notifications. Stdio app-server runtime coverage
also supports active async `process/spawn` sessions for `streamStdin` and macOS
`tty`, active `process/writeStdin`, duplicate active-handle rejection, active
`process/kill`, active `process/resizePty`, streamed process output deltas, and
process-exit notifications. Unix-socket and websocket transports reject process
