...
- set the current in-memory thread title with `/rename`
...
`/debug-config`, `/keymap`, `/rename`, `/fast`, `/copy`, `/raw`, `/vim`, `/mcp`, `!COMMAND`, `/model`, `/permissions`, `/history`, model-requested `exec_command` and
