...
fn handleAccountRateLimitsRead(allocator: std.mem.Allocator, id_value: std.json.Value, params_value: ?std.json.Value) ![]const u8 {
    ...
    const result = fetchAccountRateLimitsJson(allocator, cfg.chatgpt_base_url, credentials) catch |err| {
...
fn fetchAccountRateLimitsJson(allocator: std.mem.Allocator, base_url: []const u8, credentials: auth_mod.Credentials) ![]const u8 {
    const url = try accountRateLimitsUrl(allocator, base_url);
    ...
    return renderAccountRateLimitsResponseJson(allocator, bytes);
}

fn accountRateLimitsUrl(allocator: std.mem.Allocator, base_url: []const u8) ![]const u8 {
...
fn renderAccountRateLimitsResponseJson(allocator: std.mem.Allocator, body: []const u8) ![]const u8 {
    var parsed = try std.json.parseFromSlice(BackendRateLimitStatusPayload, allocator, body, .{ .ignore_unknown_fields = true });
    defer parsed.deinit();
    const payload = parsed.value;

    const primary = try renderRateLimitSnapshotJson(
        allocator,
        "codex",
        null,
        payload.rate_limit,
    ...
    if (payload.additional_rate_limits) |limits| {
        for (limits) |limit| {
            const snapshot = try renderRateLimitSnapshotJson(
                allocator,
...
fn renderRateLimitSnapshotJson(
    allocator: std.mem.Allocator,
    limit_id: []const u8,
    limit_name: ?[]const u8,
    ...
    const primary_json = try renderRateLimitWindowJson(allocator, if (rate_limit) |value| value.primary_window else null);
    defer allocator.free(primary_json);
    const secondary_json = try renderRateLimitWindowJson(allocator, if (rate_limit) |value| value.secondary_window else null);
...
fn renderRateLimitWindowJson(allocator: std.mem.Allocator, window: ?BackendRateLimitWindowSnapshot) ![]const u8 {
