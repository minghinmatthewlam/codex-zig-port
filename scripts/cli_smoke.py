...
def run_help_command_smoke(binary: Path) -> None:
    ...
    for argv, usage in [
        ...
        ([str(binary), "help", "plugin", "marketplace"], "codex-zig plugin marketplace [OPTIONS] <COMMAND>"),
        ([str(binary), "plugin", "help", "marketplace"], "codex-zig plugin marketplace [OPTIONS] <COMMAND>"),
...
def run_exec_server_stdio_smoke(binary: Path) -> None:
    ...
    try:
        ...
        def fs_denied_path_with_readable_target_sandbox(denied: Path, readable: Path) -> dict:
            return {
                "permissions": {
                    "type": "managed",
                    "file_system": {
                        ...
                        "entries": [
                            {"path": {"type": "path", "path": str(denied)}, "access": "none"},
                            {"path": {"type": "path", "path": str(readable)}, "access": "read"},
                        ],
                    },
                    "network": "restricted",
                },
        ...
        fs_requests = [
            ...
            {
                "jsonrpc": "2.0",
                "id": "fs-sandbox-glob-read-denied",
                "method": "fs/readFile",
                "params": {
            ...
            },
            {
                ...
                "params": {
                    "path": str(fs_glob_secret),
                    "dataBase64": base64.b64encode(b"leak").decode("ascii"),
                    "sandbox": fs_workspace_write_with_glob_deny_sandbox(fs_root),
                },
            },
            {
        ...
        if hasattr(os, "mkfifo") and hasattr(os, "symlink"):
            ...
            fs_denied_root = fs_edge_root / "denied-root"
            fs_denied_target = fs_edge_root / "denied-target"
            fs_denied_target_nested = fs_denied_target / "nested"
            try:
                ...
                fs_loop_root.mkdir()
                fs_denied_root.mkdir()
                fs_denied_target_nested.mkdir(parents=True)
                ...
                (fs_denied_target_nested / "secret.txt").write_text("denied", encoding="utf-8")
                ...
                os.symlink(fs_denied_target_nested, fs_denied_root / "alias")
                ...
                fs_edge_requests = [
                    ...
                    {
                        "jsonrpc": "2.0",
                        "id": "fs-sandbox-denied-logical-symlink-read",
                        "method": "fs/readFile",
                        "params": {
                            "path": str(fs_denied_root / "alias" / "secret.txt"),
                            "sandbox": fs_denied_path_with_readable_target_sandbox(
                                fs_denied_root, fs_denied_target_nested
                            ),
                        },
                    },
                ]
                ...
                assert len(fs_edge_responses) == 12
                ...
                assert fs_edge_by_id["fs-sandbox-denied-logical-symlink-read"]["error"]["code"] == -32600
...
def run_prompt_global_flag_smoke(binary: Path) -> None:
    ...
    try:
        ...
        for args, expected in (
            ...
            (("--strict-config", "-c", "foo.bar=1", "plugin", "--enable", "definitely-not-a-feature", "list", "--help"), "codex-zig plugin list"),
            (("--strict-config", "-c", "foo.bar=1", "plugin", "list", "--enable", "definitely-not-a-feature", "--help"), "codex-zig plugin list"),
            (("--strict-config", "-c", "foo.bar=1", "plugin", "marketplace", "--enable", "definitely-not-a-feature", "add", "--help"), "codex-zig plugin marketplace add"),
            (("--strict-config", "-c", "foo.bar=1", "plugin", "--enable", "definitely-not-a-feature", "marketplace", "add", "--help"), "codex-zig plugin marketplace add"),
            (("--strict-config", "-c", "foo.bar=1", "plugin", "--enable", "definitely-not-a-feature", "help", "list"), "codex-zig plugin list"),
            (("--strict-config", "-c", "foo.bar=1", "plugin", "--enable", "definitely-not-a-feature", "help", "marketplace", "add"), "codex-zig plugin marketplace add"),
            (("--strict-config", "-c", "foo.bar=1", "plugin", "marketplace", "--enable", "definitely-not-a-feature", "help", "add"), "codex-zig plugin marketplace add"),
            (("--strict-config", "-c", "foo.bar=1", "plugin", "help", "marketplace", "add"), "Usage:"),
            (("--strict-config", "-c", "foo.bar=1", "plugin", "marketplace", "remove", "debug", "--help"), "codex-zig plugin marketplace remove"),
        ...
        for args, expected_error in (
            ...
            (
                ("--strict-config", "-c", "foo.bar=1", "plugin", "marketplace", "add", "--sparse", "--help"),
                "`--strict-config` is not supported for `codex-zig plugin`",
            ),
            (
                ("--strict-config", "-c", "foo.bar=1", "plugin", "--enable", "definitely-not-a-feature", "help", "nope"),
                "`--strict-config` is not supported for `codex-zig plugin`",
            ),
            (
